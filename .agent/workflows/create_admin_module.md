...
- Always include `use SoftDeletes;` and add `$table->softDeletes();` in the migration.
...
Also implement the corresponding private methods that:
1. Validate and resolve record by `title`/`slug` (never trust AI-provided IDs)
2. Save a `before_state` snapshot to `ActivityLogger` before writing
3. Execute the change using the existing Model/Service (no duplicate business logic)
