...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">User Bits Balances</h1>
                <p class="text-sm text-gray-600 mt-1">Manage individual user resettable counter balances</p>
            </div>
        </div>
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
            <h3 class="text-lg font-medium text-purple-900 mb-3">Bulk Operations</h3>
            <form method="POST" action="{{ route('admin.bits-management.bulk-operation') }}" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div>
                        <label for="bulk_operation" class="block text-sm font-medium text-purple-700">Operation</label>
                    ...
                    <div>
                        <label for="bulk_counter_type_id" class="block text-sm font-medium text-purple-700">Counter Type</label>
                        <select name="counter_type_id" id="bulk_counter_type_id" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm">
                    ...
                    <div id="bulk_amount_field">
                        <label for="bulk_amount" class="block text-sm font-medium text-purple-700">Amount</label>
                        <input type="number" name="amount" id="bulk_amount" min="0" 
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm">
                    </div>
                    <div id="bulk_subscription_field" style="display: none;">
                        <label for="bulk_subscription_level_id" class="block text-sm font-medium text-purple-700">Subscription Level</label>
                    ...
                    <div>
                        <label for="bulk_reason" class="block text-sm font-medium text-purple-700">Reason</label>
                ...
                <table class="min-w-full divide-y divide-gray-300">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Counter Balances</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($users as $user)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        @forelse($user->userCounters as $userCounter)
                                            <div class="flex items-center text-sm">
                                                <span class="mr-2" style="color: {{ $userCounter->counterType->color }}">{{ $userCounter->counterType->icon }}</span>
                                                <span class="font-medium">{{ number_format($userCounter->current_count) }}</span>
                                                <span class="ml-1 text-gray-500">{{ $userCounter->counterType->name }}</span>
                                            </div>
                                        @empty
                                            <span class="text-gray-400 text-sm">No counters</span>
                                        @endforelse
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="openCounterModal('{{ $user->id }}', '{{ $user->name }}')" 
                                            class="text-indigo-600 hover:text-indigo-900 mr-3">
                                        Manage Counters
                                    </button>
                                    <a href="{{ route('admin.bits-management.transactions', ['user_id' => $user->id]) }}" 
...
<div id="counterModal" class="fixed inset-0 z-50 overflow-y-auto hidden">
                ...
                <div>
                    <div class="mt-3 text-center sm:mt-0 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                            Manage Counters for <span id="user-name"></span>
                        </h3>
                        <div class="mt-4 space-y-4">
                            <div>
                                <label for="modal_counter_type_id" class="block text-sm font-medium text-gray-700">Counter Type</label>
                                <select name="counter_type_id" id="modal_counter_type_id" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="">Select Counter</option>
                                    @foreach($counterTypes as $counterType)
                                        <option value="{{ $counterType->id }}">{{ $counterType->name }}</option>
                            ...
                            <div>
                                ...
                                <select name="action" id="modal_action" required
                                    ...
                                    <option value="add">Add Counters</option>
                                    <option value="deduct">Deduct Counters</option>
...
<script>
    ...
    document.getElementById('bulk_operation').addEventListener('change', function() {
        ...
        if (operation === 'grant_to_subscription') {
            ...
            amountField.style.display = 'block';
        } else if (operation === 'reset_counters') {
            subscriptionField.style.display = 'none';
    ...
    // Counter management modal
    function openCounterModal(userId, userName) {
        document.getElementById('user-name').textContent = userName;
        document.getElementById('counterForm').action = `/admin/bits-management/users/${userId}/update-balance`;
