...
<div class="space-y-6" x-data="bitsManagement()" x-init="init()">
        ...
        <div class="px-6 py-4">
            <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-3">
                    ...
                    <button @click="showResetScheduler = !showResetScheduler"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-cyan-500 via-blue-500 to-indigo-600 p-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-cyan-100 mb-1">Total Users</p>
                    <p class="text-3xl font-bold" x-text="formatNumber({{ $totalUsers }})">{{ number_format($totalUsers) }}</p>
                    <p class="text-xs text-cyan-200 mt-2">Active counter holders</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 via-indigo-500 to-blue-600 p-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-purple-100 mb-1">Total Transactions</p>
                    <p class="text-3xl font-bold" x-text="formatNumber({{ $totalTransactions }})">{{ number_format($totalTransactions) }}</p>
                    <p class="text-xs text-purple-200 mt-2">Counter activity</p>
                </div>
                <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                    <span class="text-3xl">📈</span>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-500 via-emerald-500 to-teal-600 p-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-green-100 mb-1">Active Counter Types</p>
                    <p class="text-3xl font-bold">{{ $counterTypes->where('is_active', true)->count() }}</p>
                    <p class="text-xs text-green-200 mt-2">Currently deployed</p>
                </div>
                <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                    <span class="text-3xl">⚡</span>
                </div>
        ...
        <div class="bg-gradient-to-br from-orange-500 via-red-500 to-pink-600 p-6 rounded-xl text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-orange-100 mb-1">Recent Activity</p>
                    <p class="text-3xl font-bold">{{ $recentTransactions->count() }}</p>
                    <p class="text-xs text-orange-200 mt-2">Last 24h transactions</p>
                </div>
                <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                    <span class="text-3xl">🔄</span>
                </div>
        ...
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-lg bg-white" @click.stop>
            <div class="mt-3">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Bulk Counter Operations</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="operation" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                <option value="reset_counters">Reset All Counters</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Counter Type</label>
                            <select name="counter_type_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">Select Counter</option>
                                @foreach($counterTypes as $counterType)
                                    <option value="{{ $counterType->id }}">{{ $counterType->name }} ({{ $counterType->icon }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    <div x-show="showResetScheduler" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" 
         x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
         class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="showResetScheduler = false">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-lg bg-white" @click.stop>
            <div class="mt-3">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Counter Reset Schedule</h3>
                    <button @click="showResetScheduler = false" class="text-gray-400 hover:text-gray-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
                ...
                <div class="space-y-4">
                    @foreach($counterTypes as $counterType)
                        @if($counterType->is_active)
                            <div class="border border-gray-200 rounded-lg p-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        <span class="text-2xl">{{ $counterType->icon }}</span>
                                        <div>
                                            <h4 class="font-medium text-gray-900">{{ $counterType->name }}</h4>
                                            <p class="text-sm text-gray-600">{{ ucfirst($counterType->reset_frequency) }} reset</p>
                                        </div>
                                    </div>
    ...
    <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
        <div class="border-b border-gray-200">
            <nav class="flex space-x-8 px-6">
                <a href="{{ route('admin.bits-management.index', ['tab' => 'overview']) }}" 
                ...
                <a href="{{ route('admin.bits-management.users') }}" 
                ...
                <a href="{{ route('admin.bits-management.transactions') }}" 
        ...
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-gray-900">Counter Performance Analytics</h3>
            ...
            <div class="space-y-6">
                @foreach($counterStats as $stat)
                    <div class="bg-gradient-to-r from-gray-50 via-white to-gray-50 border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center">
                                <div class="w-16 h-16 rounded-full flex items-center justify-center text-3xl shadow-lg" style="background: linear-gradient(135deg, {{ $stat['counterType']->color ?? '#6366f1' }}22, {{ $stat['counterType']->color ?? '#6366f1' }}44);">
                                    {{ $stat['counterType']->icon ?? '⚡' }}
                                </div>
                                <div class="ml-4">
                                    <h4 class="text-xl font-bold text-gray-900">{{ $stat['counterType']->name }}</h4>
                                    <p class="text-sm text-gray-600 max-w-md">{{ $stat['counterType']->description }}</p>
                                    <div class="flex items-center mt-2 space-x-3">
                                        @if($stat['counterType']->is_active)
                                        ...
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ ucfirst($stat['counterType']->reset_frequency) }} Reset
                                        </span>
                                        <span class="text-xs text-gray-500">
                                            Base: {{ number_format($stat['counterType']->default_allocation) }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-white p-4 rounded-lg shadow-sm border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-indigo-600 mt-1">{{ number_format($stat['total_allocation']) }}</p>
                                    </div>
                            ...
                            <div class="bg-white p-4 rounded-lg shadow-sm border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-green-600 mt-1">{{ number_format($stat['users_with_counters']) }}</p>
                                    </div>
                            ...
                            <div class="bg-white p-4 rounded-lg shadow-sm border">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold text-purple-600 mt-1">{{ $totalUsers > 0 ? number_format(($stat['users_with_counters'] / $totalUsers) * 100, 1) : 0 }}%</p>
                                    </div>
    ...
    <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-xl font-bold text-gray-900">Live Counter Activity</h3>
                <div class="flex items-center space-x-2">
        ...
        <div class="p-6">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2 mt-1">
                                        <span class="text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $transaction->type)) }}</span>
                                        <span class="text-gray-400">•</span>
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="text-right">
                                    <div class="flex items-center space-x-2">
                                        <span class="text-2xl">{{ $transaction->counterType->icon ?? '⚡' }}</span>
                                        <span class="font-medium text-gray-900">{{ $transaction->counterType->name }}</span>
                                    </div>
                                    <div class="flex items-center justify-end space-x-2 mt-1">
                                        <span class="text-lg font-bold {{ $transaction->count_change >= 0 ? 'text-green-600' : 'text-red-600' }}">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-4">Counter transactions will appear here as users interact with the bits system.</p>
