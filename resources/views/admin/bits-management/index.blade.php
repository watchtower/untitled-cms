...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Bits Management</h1>
                <p class="text-sm text-gray-600 mt-1">Manage resettable counters and user allocations</p>
            </div>
        </div>

        <!-- Statistics Overview -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-gradient-to-r from-cyan-500 to-blue-600 p-4 rounded-lg text-white">
            ...
            <div class="bg-gradient-to-r from-purple-500 to-indigo-600 p-4 rounded-lg text-white">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-purple-100">Total Transactions</p>
            ...
            <div class="bg-gradient-to-r from-green-500 to-teal-600 p-4 rounded-lg text-white">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-green-100">Active Counter Types</p>
                        <p class="text-2xl font-bold">{{ $counterTypes->where('is_active', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-orange-500 to-red-500 p-4 rounded-lg text-white">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-sm font-medium text-orange-100">Recent Activity</p>
                        <p class="text-2xl font-bold">{{ $recentTransactions->count() }}</p>
                        <p class="text-xs text-orange-100">Last 24h</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <h3 class="text-lg font-medium text-gray-900">Counter Type Statistics</h3>
            
            @foreach($counterStats as $stat)
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <span class="text-2xl mr-3" style="color: {{ $stat['counterType']->color }}">{{ $stat['counterType']->icon }}</span>
                            <div>
                                <h4 class="text-lg font-semibold text-gray-900">{{ $stat['counterType']->name }}</h4>
                                <p class="text-sm text-gray-600">{{ $stat['counterType']->description }}</p>
                        ...
                        <div class="text-right">
                            <div class="grid grid-cols-3 gap-4 text-center">
                                <div>
                                    <p class="text-2xl font-bold text-indigo-600">{{ number_format($stat['total_allocation']) }}</p>
                                    <p class="text-xs text-gray-500">Total Allocated</p>
                                </div>
                                <div>
                                    <p class="text-2xl font-bold text-green-600">{{ number_format($stat['users_with_counters']) }}</p>
        ...
        <div class="mt-8">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center">
                                            <span class="mr-2" style="color: {{ $transaction->counterType->color }}">{{ $transaction->counterType->icon }}</span>
                                            <span class="text-sm text-gray-900">{{ $transaction->counterType->name }}</span>
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm font-medium {{ $transaction->isAddition() ? 'text-green-600' : 'text-red-600' }}">
                                            {{ $transaction->formatted_count_change }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $transaction->reason }}</div>
                                        <div class="text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $transaction->type)) }}</div>
                                    </td>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Counter transactions will appear here when users start using Bits.</p>
                </div>
            @endif
        </div>
    </div>
