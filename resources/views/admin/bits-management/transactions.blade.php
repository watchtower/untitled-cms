...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Bits Transaction Log</h1>
                <p class="text-sm text-gray-600 mt-1">Complete history of all resettable counter transactions</p>
            </div>
        </div>
        ...
        <div class="bg-gray-50 p-4 rounded-lg mb-6">
            <form method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    ...
                    <div>
                        <label for="counter_id" class="block text-sm font-medium text-gray-700">Counter Type</label>
                        <select name="counter_id" id="counter_id" 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            <option value="">All Counter Types</option>
                            @foreach($counterTypes as $counterType)
                                <option value="{{ $counterType->id }}" {{ request('counter_id') == $counterType->id ? 'selected' : '' }}>
                                    {{ $counterType->name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <!-- Summary Statistics -->
        @if(request()->hasAny(['user_id', 'counter_id', 'type', 'date_from', 'date_to']))
                ...
                <div class="bg-green-50 p-4 rounded-lg">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-2xl font-bold text-green-900">{{ number_format($transactions->where('count_change', '>', 0)->count()) }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-red-50 p-4 rounded-lg">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-2xl font-bold text-red-900">{{ number_format($transactions->where('count_change', '<', 0)->count()) }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-purple-50 p-4 rounded-lg">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-2xl font-bold text-purple-900">{{ number_format($transactions->sum('count_change')) }}</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <div class="flex items-center space-x-2">
                                        <span class="text-gray-500">{{ number_format($transaction->count_before) }}</span>
                                        <span class="text-gray-400">→</span>
                                        <span class="font-medium">{{ number_format($transaction->count_after) }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                                 @switch($transaction->type)
                                                     ...
                                                     @case('automatic_reset') bg-blue-100 text-blue-800 @break
                                                     @case('usage') bg-orange-100 text-orange-800 @break
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    ...
                    @else
                        Transactions will appear here when users start using Bits.
                    @endif
                </p>
            </div>
        @endif
